...
const handleDeleteImage = (indexid) =>{
  ...
  setFormData(prev => ({
    ...prev,
    imageUrls: prev.imageUrls.filter((_, index) => index !== indexid)
  }));
}
  ...
  filesArray.forEach((file, index) => {
    ...
    reader.onload = () => {
      ...
      if (allBase64Images.length === filesArray.length) {
        ...
        setFormData(prev=>({
          ...prev, 
          imageUrls: [...(prev.imageUrls || []), ...allBase64Images]
        }));
        e.target.value = '';
      }
    };
...
<button className='bg-green-600 p-4 md:p-3 md:w-[350px] m-1 rounded-full font-bold text-white text-xl cursor-pointer disabled:opacity-80' disabled={loading}>{loading ? "Edit Listing...." : "Edit Listing"}</button>
</div>
    </form>
   </main>
  )
